...
from aws.auth import AwsRuntime, build_aws_runtime, resolve_aws_region
from aws.sparkplug_shadow import (
...
RIG_TYPE_THING_GROUP_PREFIX = "txing-rig-type-"
...
def _rig_type_group_name(rig_type: str) -> str:
    return f"{RIG_TYPE_THING_GROUP_PREFIX}{_normalize_slug('rig type', rig_type)}"
...
class EnlistService:
    ...
    def _list_thing_group_names(self, thing_name: str) -> tuple[str, ...]:
        names: set[str] = set()
        next_token: str | None = None
        while True:
            ...
            if next_token:
                request["nextToken"] = next_token
            response = self._iot.list_thing_groups_for_thing(**request)
            for group in response.get("thingGroups", []):
                if isinstance(group, Mapping) and isinstance(group.get("groupName"), str):
    ...
    def _ensure_rig_type_group_membership(self, thing_name: str, rig_type: str) -> str:
        target = _rig_type_group_name(rig_type)
        try:
            self._iot.create_thing_group(thingGroupName=target)
        except Exception as error:
            if _error_code(error) != "ResourceAlreadyExistsException":
                raise
        for group_name in self._list_thing_group_names(thing_name):
            if group_name.startswith(RIG_TYPE_THING_GROUP_PREFIX) and group_name != target:
                try:
                    self._iot.remove_thing_from_thing_group(
                    ...
                    if not _is_resource_not_found(error):
                        raise
        self._iot.add_thing_to_thing_group(
            thingName=thing_name,
    ...
    def enlist_rig(self, *, town_id: str, rig_type: str, rig_name: str) -> EnlistResult:
        ...
        thing_name = _require_text(thing, "thingName", context="rig thing")
        initialized = self._initialize_rig_shadows(
            thing_name,
            town_id=normalized_town_id,
            rig_id=thing_name,
        )
        group_name = self._ensure_rig_type_group_membership(
            thing_name,
            normalized_rig_type,
        )
        return self._thing_result(
            ...
            attributes=attributes,
            initialized_shadows=initialized,
            auxiliary_resources={"thingGroupName": group_name},
        )
...
def _error_type(error: Exception) -> str:
    if isinstance(error, EnlistError):
        return "EnlistError"
    return "Error"

...
class AwsIotWebsocketConnection(_AwsIotWebsocketConnectionBase):
...
class AwsIotWebsocketSyncConnection(_AwsIotWebsocketConnectionBase):
    def connect(self, *, timeout_seconds: float | None = None) -> Any:
        return _future_result(
            self._connection.connect(),
            timeout_seconds=timeout_seconds or self._config.connect_timeout_seconds,
        )

    def disconnect(self, *, timeout_seconds: float | None = None) -> Any:
        return _future_result(
            self._connection.disconnect(),
            timeout_seconds=timeout_seconds or self._config.connect_timeout_seconds,
        )
    ...
    def resubscribe_existing_topics(
        self,
        *,
        timeout_seconds: float | None = None,
    ) -> Any:

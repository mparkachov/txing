...
from .auth import build_aws_runtime, resolve_aws_region
...
RIG_TYPE_DEFINITIONS: dict[str, RigTypeDefinition] = {
    ...
    "local": RigTypeDefinition(
        rig_type="local",
        display_name="Local Rig",
        default_name="dev",
        capabilities=("sparkplug",),
        redcon_command_levels=("1", "4"),
    ),
...
def build_type_records(*, repo_root: Path | None = None) -> dict[str, dict[str, Any]]:
    ...
    for device_type in ("unit", "cloud-mcu", "weather", "power", "power-si", "mac"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not region_name:
        raise RuntimeError("AWS region is required; configure an AWS CLI/SDK region or pass --region")

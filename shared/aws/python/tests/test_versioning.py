...
class VersionEnvironmentTests(unittest.TestCase):
    ...
    def test_unit_daemon_manual_docker_build_replaces_release_channel(self) -> None:
        ...
        self.assertFalse((daemon_dir / "Cargo.toml").exists())
        self.assertFalse((daemon_dir / "Cargo.lock").exists())
        self.assertFalse((daemon_dir / "build.rs").exists())
        self.assertFalse((daemon_dir / "src" / "lib.rs").exists())
        self.assertFalse((daemon_dir / "src" / "main.rs").exists())
        ...
        self.assertNotIn("txing-cargo-registry", justfile)
        self.assertNotIn("txing-cargo-git", justfile)
        self.assertNotIn("CARGO_TARGET_DIR", justfile)

...
class VersionEnvironmentTests(unittest.TestCase):
    ...
    def test_stable_release_publishes_only_project_assets(self) -> None:
        ...
        self.assertIn("KVS_MASTER_OUTPUT_DIR: .txing-workflow-out", workflow)
        self.assertIn("Build native KVS master in Trixie container", workflow)
        self.assertIn("docker run --rm", workflow)
        self.assertIn('install -d "$KVS_MASTER_OUTPUT_DIR"', workflow)
        self.assertIn('-e KVS_MASTER_OUTPUT_DIR="$KVS_MASTER_OUTPUT_DIR"', workflow)
        self.assertIn("just unit::board::build-native", workflow)
        self.assertIn("archive.raspberrypi.com/debian/raspberrypi.gpg.key", workflow)
        ...
        self.assertIn('install -m 755 devices/unit/board/kvs_master/build/txing-board-kvs-master "$KVS_MASTER_OUTPUT_DIR/$KVS_MASTER_BINARY"', workflow)
        self.assertIn('package_binary "$KVS_MASTER_OUTPUT_DIR/$KVS_MASTER_BINARY"', workflow)
        self.assertNotIn('-v "$RUNNER_TEMP:/out"', workflow)
        self.assertNotIn('"/out/$KVS_MASTER_BINARY"', workflow)
        self.assertNotIn('"$RUNNER_TEMP/txing-board-kvs-master"', workflow)
    ...
    def test_unit_daemon_feature_prerelease_publishes_daemon_and_kvs_master(self) -> None:
        ...
        self.assertNotIn('-v "$RUNNER_TEMP:/out"', workflow)
        self.assertNotIn('"/out/$KVS_MASTER_BINARY"', workflow)
        self.assertNotIn('"$RUNNER_TEMP/$KVS_MASTER_BINARY"', workflow)
    ...
    def test_unit_daemon_root_owned_installer_uses_mise_for_daemon_and_kvs_master(self) -> None:
        ...
        self.assertIn("MISE_SHARED_INSTALL_DIRS=$root_home/.local/share/mise/installs", installer)
    ...
    def test_board_stable_docs_use_daemon_kvs_master_release_path(self) -> None:
        ...
        self.assertIn("MISE_SHARED_INSTALL_DIRS=/root/.local/share/mise/installs", artifacts_docs)
        ...
        self.assertNotIn("MISE_SHARED_INSTALL_DIRS=/home/txing/.local/share/mise/installs", artifacts_docs)

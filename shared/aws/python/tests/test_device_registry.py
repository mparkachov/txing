    ...
    def search_index(self, **kwargs: object) -> dict[str, object]:
        ...
        for thing_name, thing in self._things.items():
            ...
            if "thingTypeName:raspi" in query and thing_type != "raspi":
                continue
            if "thingTypeName:cloud" in query and thing_type != "cloud":
                continue
            for key in ("name", "townId", "rigId"):
...
class DeviceRegistryTests(unittest.TestCase):
    def test_register_town_writes_minimal_instance_attributes(self) -> None:
    ...
    def test_register_cloud_rig_uses_rig_type_as_thing_type(self) -> None:
        runtime = _FakeRuntime()
        registry = AwsDeviceRegistry(
            runtime,
            repo_root=REPO_ROOT,
        ...
        self.assertEqual(registration.thing_name, "cloud-rig002")
        self.assertEqual(runtime.iot.create_thing_requests[0]["thingTypeName"], "cloud")
        attributes = runtime.iot.create_thing_requests[0]["attributePayload"]["attributes"]  # type: ignore[index]
        self.assertEqual(
            attributes,
            {
    ...
    def test_device_enrollment_is_checked_through_ssm_path_compatibility(self) -> None:
        ...
        self.assertNotIn("deviceType", attributes)
    ...
    def test_register_power_nrf_uses_thread_capability_catalog(self) -> None:
        runtime = _FakeRuntime()
        registry = AwsDeviceRegistry(
            runtime,
            repo_root=REPO_ROOT,
            random_source=_SequenceRandom("pn0001"),
        )

        registration = registry.register_device(rig_id="raspi-ras001", device_type="power-nrf")

        self.assertEqual(registration.thing_name, "power-nrf-pn0001")
        self.assertEqual(registration.capabilities, ("sparkplug", "thread", "power"))
        self.assertEqual(runtime.iot.create_thing_requests[0]["thingTypeName"], "power-nrf")
        self.assertEqual(
            sorted(shadow_name for _thing_name, shadow_name in runtime.iot_data.shadows),
            ["power", "sparkplug", "thread"],
        )
    ...
    def test_register_tbot_initializes_mavlink_shadow_and_both_signaling_channels(self) -> None:
        runtime = _FakeRuntime()
        registry = AwsDeviceRegistry(
            runtime,
            repo_root=REPO_ROOT,
            random_source=_SequenceRandom("tbot01"),
        )
        ...
        self.assertEqual(
            runtime.kinesisvideo.channels,
            {"tbot-tbot01-board-video", "tbot-tbot01-mavlink"},
        )
        self.assertEqual(
            [request["ChannelName"] for request in runtime.kinesisvideo.create_requests],
            ["tbot-tbot01-board-video", "tbot-tbot01-mavlink"],
    ...
    def test_assign_device_validates_target_rig_type_before_updating_parent_ids(self) -> None:
        ...
        self.assertEqual(registration.rig_id, "raspi-ras002")
        self.assertEqual(

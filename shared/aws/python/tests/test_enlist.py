...
class EnlistServiceTests(unittest.TestCase):
    ...
    def test_enlist_cyberbrick_creates_mavlink_shadow_and_independent_signaling_channels(self) -> None:
        ...
        result = self._enlist_device(raspi["thingName"], "cyberbrick", "rover")

        self.assertEqual(result["thingTypeName"], "cyberbrick")
        self.assertEqual(
            result["attributes"]["capabilities"],
    ...
    def test_enlist_power_nrf_initializes_thread_and_power_shadows(self) -> None:
        town = self._enlist_town()
        raspi = self._enlist_rig(town["thingName"], "raspi", "server")

        result = self._enlist_device(raspi["thingName"], "power-nrf", "power-nrf")

        self.assertEqual(result["thingTypeName"], "power-nrf")
        self.assertEqual(result["attributes"]["kind"], "deviceType")
        self.assertEqual(result["attributes"]["rigType"], "raspi")
        self.assertEqual(result["attributes"]["deviceType"], "power-nrf")
        self.assertEqual(result["attributes"]["webAdapter"], "web/power-nrf-adapter.tsx")
        self.assertEqual(result["attributes"]["capabilities"], "sparkplug,thread,power")
        self.assertEqual(result["attributes"]["redconCommandLevels"], "4,3")
        self.assertEqual(result["initializedShadows"], ["sparkplug", "thread", "power"])

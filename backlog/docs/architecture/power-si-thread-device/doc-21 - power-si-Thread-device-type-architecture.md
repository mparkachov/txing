...
Firmware targets the stock Zephyr board `xiao_mg24` using the repo stock Zephyr workflow and does not enable Matter. The implementation builds as a Thread MTD Sleepy End Device with a `5000 ms` poll period so it matches the original low-power Thread intent while keeping bounded rig command latency. On XIAO MG24, runtime startup uses a temporary receiver-on SRP bootstrap mode to attach and complete service registration on the current Zephyr/Silabs stack, then updates the attached child into steady-state SED link mode with `mRxOnWhenIdle=false` and full network data after SRP acceptance. If the current Zephyr/Silabs SED path does not remain attached, firmware restarts Thread back into receiver-on bootstrap mode for the rest of that boot to keep the Thread service usable while SED acceptance remains open. Hardware evidence narrowed the remaining stock-Zephyr blocker to the Silabs `IEEE802154_HW_TX_SEC` path for secured sleepy MAC Data Request frames; SED validation and current measurement therefore use explicit local test profiles that temporarily enable OpenThread software MAC TX security during the build while the upstream issue is open.

...
For TLS trust on the KVS signaling path, the board runtime relies on the platform OpenSSL trust store. `board-video-sender` strips inherited CA override environment variables before launching the native sender, and the board-local AWS KVS SDK build no longer bakes in a repo-local or Debian-specific CA bundle path. Install normal OS trust data such as Debian's `ca-certificates` package. You do not need repo-local AWS root CA files or IoT-specific certificate files for the txing runtime bootstrap.

...
#define WEATHER_MEASUREMENT_UUID_VAL                                                         \
...
static uint8_t weather_measurement_payload[WEATHER_MEASUREMENT_PAYLOAD_SIZE] = {
	WEATHER_PROTOCOL_VERSION,
};
...
static const struct bt_uuid_128 weather_measurement_uuid =
	BT_UUID_INIT_128(WEATHER_MEASUREMENT_UUID_VAL);

static void encode_weather_state(uint8_t redcon, bool bme280_valid, uint16_t battery_mv)
{
	uint8_t flags = 0U;
			     ...
			     const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	/*
	 * This profile is only for connectable idle-power testing. It accepts
	 * the command payload so central-side write paths can be exercised, but
	 * it does not wake the board, enable D1 power, or synthesize telemetry.
	 */
	encode_weather_state(WEATHER_REDCON_IDLE, false, 0U);
...
BT_GATT_SERVICE_DEFINE(weather_svc,
	...
	BT_GATT_CHARACTERISTIC(&weather_state_uuid.uuid, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
			       BT_GATT_PERM_READ, read_payload, NULL, &weather_state_value),
	BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
	BT_GATT_CHARACTERISTIC(&weather_measurement_uuid.uuid,
			       BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
			       BT_GATT_PERM_READ, read_payload, NULL, &weather_measurement_value),
	BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),
);

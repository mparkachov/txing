...
func testFileEnv() map[string]string {
	return map[string]string{
		"TXING_THING_ID":                "cyberbrick-local",
		...
		"TXING_IOT_ROLE_ALIAS":          "cyberbrick-daemon-role-alias",
		"TXING_IOT_CERT_FILE":           "/home/txing/.config/txing/cyberbrick-daemon/certificate.pem.crt",
		"TXING_IOT_PRIVATE_KEY_FILE":    "/home/txing/.config/txing/cyberbrick-daemon/private.pem.key",
		"TXING_IOT_ROOT_CA_FILE":        "/home/txing/.config/txing/cyberbrick-daemon/AmazonRootCA1.pem",
	}
}
...
func TestParseEnvFileWithoutShellExecution(t *testing.T) {
	...
	if parsed["TXING_THING_ID"] != "cyberbrick-local" || parsed["AWS_REGION"] != "eu-central-1" || parsed["TXING_IOT_ROLE_ALIAS"] != "alias-name" || parsed["EMPTY"] != "" {
		t.Fatalf("unexpected parsed env: %#v", parsed)
	}
	if _, err := ParseEnvFileContents("export TXING_THING_ID=cyberbrick-local"); err == nil {
...
func TestLoadsEnvFileFromConfigDir(t *testing.T) {
	...
	if err := os.WriteFile(envFile, []byte("TXING_THING_ID=cyberbrick-local\nAWS_REGION=eu-central-1\n"), 0o600); err != nil {
	...
	if loaded.Path != envFile || loaded.Values["TXING_THING_ID"] != "cyberbrick-local" {
...
func TestColocatedCertPathsDefaultToEnvFileDirectory(t *testing.T) {
	...
	cli, err := ParseCLI([]string{"--client-id", "cyberbrick-local-daemon-test"})
	if err != nil {
		t.Fatalf("parse cli: %v", err)
	}
	config, err := RuntimeConfigFromSourcesWithEnvFileDir(cli, map[string]string{}, fileEnv, "/home/txing/.config/txing/cyberbrick-daemon")
	if err != nil {
		t.Fatalf("runtime config: %v", err)
	}
	if config.IoTCertFile != "/home/txing/.config/txing/cyberbrick-daemon/certificate.pem.crt" {
		t.Fatalf("unexpected cert file %q", config.IoTCertFile)
	}
	if config.IoTPrivateKeyFile != "/home/txing/.config/txing/cyberbrick-daemon/private.pem.key" {
		t.Fatalf("unexpected private key file %q", config.IoTPrivateKeyFile)
	}
	if config.IoTRootCAFile != "/home/txing/.config/txing/cyberbrick-daemon/AmazonRootCA1.pem" {
...
func TestCloudWatchConfigUsesPrecedenceAndDefaults(t *testing.T) {
	fileEnv := testFileEnv()
	fileEnv["TXING_CLOUDWATCH_LOG_GROUP"] = "txing/town-file/rig-file/cyberbrick-local"
	...
	cli := CLI{
		...
		CloudWatchLogGroup:         "txing/town-cli/rig-cli/cyberbrick-local",
	...
	expected := &CloudWatchLogConfig{
		LogGroup:      "txing/town-cli/rig-cli/cyberbrick-local",
...
func TestCLIDefaultsToDeclaredCyberbrickCapabilities(t *testing.T) {
	config := runtimeConfigFromArgs(t)
	if config.ThingID != "cyberbrick-local" {
	...
	if config.VideoChannelName != "cyberbrick-local-board-video" || config.HardwareWorkerSocketPath != DefaultHardwareSocketPath {
		t.Fatalf("default runtime config mismatch: %#v", config)
	}
	if !strings.HasPrefix(config.ClientID, "cyberbrick-local-daemon-") {
...
func TestDefaultClientIDSanitizesThingName(t *testing.T) {
	if got := defaultClientID("Cyberbrick Local!", 42); got != "Cyberbrick-Local-daemon-42" {

...
import (
	...
	boardvideov1 "github.com/mparkachov/txing/devices/cyberbrick/daemon/internal/proto/boardvideov1"
	hardwarev1 "github.com/mparkachov/txing/devices/cyberbrick/daemon/internal/proto/hardwarev1"
...
func testRuntimeConfig() RuntimeConfig {
	return RuntimeConfig{
		...
		IoTRoleAlias:               "cyberbrick-daemon-role-alias",
		IoTCertFile:                "/home/txing/.config/txing/cyberbrick-daemon/certificate.pem.crt",
		IoTPrivateKeyFile:          "/home/txing/.config/txing/cyberbrick-daemon/private.pem.key",
		IoTRootCAFile:              "/home/txing/.config/txing/cyberbrick-daemon/AmazonRootCA1.pem",
		ClientID:                   "cyberbrick-local-daemon-test",
		...
		VideoChannelName:           "cyberbrick-local-board-video",
...
func TestRuntimePublishesBoardShadowMCPVideoAndCapabilityPayloads(t *testing.T) {
	...
	expectedTopics := []string{
		"$aws/things/cyberbrick-local/shadow/name/board/update",
		"txings/cyberbrick-local/mcp/descriptor",
		"txings/cyberbrick-local/mcp/status",
		"$aws/things/cyberbrick-local/shadow/name/mcp/update",
		"txings/cyberbrick-local/video/descriptor",
		"txings/cyberbrick-local/video/status",
		"$aws/things/cyberbrick-local/shadow/name/video/update",
		"txings/cyberbrick-local/capability/v2/state",
		"txings/cyberbrick-local/mcp/status",
		"$aws/things/cyberbrick-local/shadow/name/mcp/update",
		"txings/cyberbrick-local/capability/v2/state",
		"$aws/things/cyberbrick-local/shadow/name/board/update",
...
func TestRetainedDynamicExpiryUsesConfiguredCapabilityTTL(t *testing.T) {
	...
	mcpDescriptor := findPublishedMessage(t, messages, "txings/cyberbrick-local/mcp/descriptor")
	mcpStatus := findPublishedMessage(t, messages, "txings/cyberbrick-local/mcp/status")
	videoDescriptor := findPublishedMessage(t, messages, "txings/cyberbrick-local/video/descriptor")
	videoStatus := findPublishedMessage(t, messages, "txings/cyberbrick-local/video/status")
	capabilityState := findPublishedMessage(t, messages, "txings/cyberbrick-local/capability/v2/state")
...
func TestMQTTConnectPacketUsesMQTT5CleanStartAndSessionExpiry(t *testing.T) {
	packetType, flags, payload, err := readMQTTPacket(bytes.NewReader(mqttPacketConnect("cyberbrick-local")))
	if err != nil {
	...
	if clientIDStart+clientIDLength > len(payload) || string(payload[clientIDStart:clientIDStart+clientIDLength]) != "cyberbrick-local" {
...
func TestMQTTPublishPacketEncodesMQTT5MessageExpiry(t *testing.T) {
	expiry := uint32(150)
	packetType, flags, payload, err := readMQTTPacket(bytes.NewReader(mqttPacketPublish(9, "txings/cyberbrick-local/video/status", []byte("status"), true, &expiry)))
	if err != nil {
	...
	topicEnd := 2 + topicLen
	if string(payload[2:topicEnd]) != "txings/cyberbrick-local/video/status" {
	...
	if !ok || topic != "txings/cyberbrick-local/video/status" || packetID != 9 || qos != 1 || string(body) != "status" {
...
func TestMQTTSubscribeAndPubAckUseMQTT5Properties(t *testing.T) {
	packetType, flags, payload, err := readMQTTPacket(bytes.NewReader(mqttPacketSubscribe(4, "txings/cyberbrick-local/mcp/session/+/c2s")))
	if err != nil {
...
func TestMQTTMCPRequestsAreRejectedWhileWebRTCOnlyIsAdvertised(t *testing.T) {
	...
	topic := "txings/cyberbrick-local/mcp/session/session-a/c2s"
...
func TestBoardVideoBridgeWorkerConfigAndUnixSocketEvents(t *testing.T) {
	...
	if workerConfig.GetRegion() != "eu-central-1" || workerConfig.GetChannelName() != "cyberbrick-local-board-video" || workerConfig.GetClientId() != "cyberbrick-local-cyberbrick-kvs-master" || !workerConfig.GetPreferIpv6() {
...
func TestGRPCHardwareClientDelegatesOverUnixSocket(t *testing.T) {
	...
	hardwarev1.RegisterCyberbrickHardwareServer(server, fake)
	go server.Serve(listener)
	defer server.Stop()
...
func TestIotCredentialAndSparkplugHelpers(t *testing.T) {
	config := testRuntimeConfig()
	config.IoTRoleAlias = "cyberbrick_daemon,role-alias@Test=1"
	request, err := BuildIotCredentialsRequest(config)
	if err != nil {
		t.Fatalf("build request: %v", err)
	}
	if request.URL != "https://example.credentials.iot.eu-central-1.amazonaws.com/role-aliases/cyberbrick_daemon,role-alias@Test=1/credentials" || request.ThingName != "cyberbrick-local" {
...
func TestCloudWatchWriterSemantics(t *testing.T) {
	...
	if !reflect.DeepEqual(fake.calls, []string{
		"create-log-group:txing/town-local/rig-local/cyberbrick-local",
		"put-retention:txing/town-local/rig-local/cyberbrick-local:14",
		"create-log-stream:txing/town-local/rig-local/cyberbrick-local:daemon/cyberbrick-local-daemon-test",
	}) {
...
type fakeHardwareServer struct {
	hardwarev1.UnimplementedCyberbrickHardwareServer
	lastDeadline  uint64
...
func latestMCPStatus(t *testing.T, messages []PublishedMessage) map[string]interface{} {
	...
	for index := len(messages) - 1; index >= 0; index-- {
		if messages[index].Topic != "txings/cyberbrick-local/mcp/status" {
			continue
		}
...
func cloudWatchLogConfigForRuntimeTest() CloudWatchLogConfig {
	return CloudWatchLogConfig{
		LogGroup:      "txing/town-local/rig-local/cyberbrick-local",
		LogStream:     "daemon/cyberbrick-local-daemon-test",

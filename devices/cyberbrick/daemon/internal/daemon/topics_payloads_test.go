...
func TestBuildsRetainedCapabilityStateTopic(t *testing.T) {
	topic, err := BuildCapabilityStateTopic("cyberbrick-local")
	if err != nil {
		t.Fatalf("build topic: %v", err)
	}
	if topic != "txings/cyberbrick-local/capability/v2/state" {
		t.Fatalf("unexpected topic %q", topic)
	}
	if _, err := BuildCapabilityStateTopic("cyberbrick/local"); err == nil {
...
func TestBuildsPublicTopics(t *testing.T) {
	cases := map[string]func() (string, error){
		"mcp root":         func() (string, error) { return BuildMCPTopicRoot("cyberbrick-local") },
		"mcp descriptor":   func() (string, error) { return BuildMCPDescriptorTopic("cyberbrick-local") },
		"mcp status":       func() (string, error) { return BuildMCPStatusTopic("cyberbrick-local") },
		"video root":       func() (string, error) { return BuildVideoTopicRoot("cyberbrick-local") },
		"video descriptor": func() (string, error) { return BuildVideoDescriptorTopic("cyberbrick-local") },
		"video status":     func() (string, error) { return BuildVideoStatusTopic("cyberbrick-local") },
		"mcp c2s sub":      func() (string, error) { return BuildMCPSessionC2SSubscription("cyberbrick-local") },
		"mcp s2c":          func() (string, error) { return BuildMCPSessionS2CTopic("cyberbrick-local", "session-a") },
		"board shadow":     func() (string, error) { return BuildBoardShadowUpdateTopic("cyberbrick-local") },
		"mcp shadow":       func() (string, error) { return BuildMCPShadowUpdateTopic("cyberbrick-local") },
		"video shadow":     func() (string, error) { return BuildVideoShadowUpdateTopic("cyberbrick-local") },
	}
	expected := map[string]string{
		"mcp root":         "txings/cyberbrick-local/mcp",
		"mcp descriptor":   "txings/cyberbrick-local/mcp/descriptor",
		"mcp status":       "txings/cyberbrick-local/mcp/status",
		"video root":       "txings/cyberbrick-local/video",
		"video descriptor": "txings/cyberbrick-local/video/descriptor",
		"video status":     "txings/cyberbrick-local/video/status",
		"mcp c2s sub":      "txings/cyberbrick-local/mcp/session/+/c2s",
		"mcp s2c":          "txings/cyberbrick-local/mcp/session/session-a/s2c",
		"board shadow":     "$aws/things/cyberbrick-local/shadow/name/board/update",
		"mcp shadow":       "$aws/things/cyberbrick-local/shadow/name/mcp/update",
		"video shadow":     "$aws/things/cyberbrick-local/shadow/name/video/update",
	}
	...
	sessionID, ok := ParseMCPSessionC2STopic("cyberbrick-local", "txings/cyberbrick-local/mcp/session/session-a/c2s")
	if !ok || sessionID != "session-a" {
		t.Fatalf("unexpected parsed session id: %q ok=%v", sessionID, ok)
	}
	if _, ok := ParseMCPSessionC2STopic("cyberbrick-local", "txings/cyberbrick-local/mcp/session/bad/id/c2s"); ok {
...
func TestBuildsCapabilityPayloadWithTTLAndExpiredState(t *testing.T) {
	...
	if payload.SchemaVersion != SchemaVersion || payload.AdapterID != AdapterID || payload.ThingName != "cyberbrick-local" {
...
func TestBuildsStaticMCPAndVideoDescriptors(t *testing.T) {
	...
	if !jsonContains(encoded, `"activeTtlMs":5000`) || !jsonContains(encoded, `"topicRoot":"txings/cyberbrick-local/mcp"`) {
		t.Fatalf("mcp descriptor missing static fields: %s", encoded)
	}
	videoDescriptor := VideoDescriptor(config)
	if videoDescriptor["transport"] != DefaultVideoTransport || videoDescriptor["channelName"] != "cyberbrick-local-board-video" || videoDescriptor["region"] != "eu-central-1" {

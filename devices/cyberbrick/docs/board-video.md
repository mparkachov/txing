...
- Field-validation status: the unit parity baseline was accepted for the
  plain-AWS-WebRTC path; Cyberbrick physical-board validation remains pending
- Runtime shape: `txing-cyberbrick-daemon` and the native
  `txing-cyberbrick-kvs-master` run as separately supervised OpenRC services. The daemon serves
...
- `txing-cyberbrick-daemon` publishes board power and wifi state for the stable Go
  runtime path.
- `txing-cyberbrick-daemon` publishes retained video descriptor/status topics under `txings/<device_id>/video/*`.
...
- The native sender implementation is shipped in-tree and packaged as the
  `txing-cyberbrick-kvs-master` release asset. The daemon and worker communicate
...
native sender command
  -> is shipped as txing-cyberbrick-kvs-master
...
- a repo-managed native sender that runs as its own OpenRC service by default
...
The equivalent unit implementation has been accepted through manual field
validation from a business perspective. This establishes the behavior parity
baseline; it is not evidence for Cyberbrick's Alpine hardware integration.

What is recorded for that acceptance:

- the unit plain AWS WebRTC operator path was manually exercised in realistic use
- practical directional-control quality was considered good enough for business use
- no second direct operator path was justified by that unit validation
...
Cyberbrick still requires the milestone's physical-board smoke test for camera
enumeration, H.264 capture, end-to-end operator video, and motor parity.

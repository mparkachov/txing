...
#include "txing/cyberbrick/hardware/v1/cyberbrick_hardware.grpc.pb.h"
...
namespace hw = txing::cyberbrick::hardware_worker;
namespace cyberbrick_hw_pb = txing::cyberbrick::hardware::v1;
...
cyberbrick_hw_pb::MotionState ToProto(const hw::MotionState& motion) {
    cyberbrick_hw_pb::MotionState out;
...
cyberbrick_hw_pb::HardwareState ToProto(hw::HardwareState state) {
    switch (state) {
        case hw::HardwareState::Starting:
            return cyberbrick_hw_pb::STARTING;
        case hw::HardwareState::Ready:
            return cyberbrick_hw_pb::READY;
        case hw::HardwareState::Degraded:
            return cyberbrick_hw_pb::DEGRADED;
        case hw::HardwareState::Error:
            return cyberbrick_hw_pb::ERROR;
        case hw::HardwareState::Stopped:
            return cyberbrick_hw_pb::STOPPED;
    }
    return cyberbrick_hw_pb::HARDWARE_STATE_UNSPECIFIED;
}

cyberbrick_hw_pb::HardwareStatus ToProto(const hw::HardwareStatus& status) {
    cyberbrick_hw_pb::HardwareStatus out;
    ...
    *out.mutable_motion() = ToProto(status.motion);
    ...
    out.set_worker_version(hw::kTxingCyberbrickHardwareWorkerVersion);
    return out;
}

hw::Vector3 FromProto(const cyberbrick_hw_pb::Vector3& value) {
    hw::Vector3 out;
    out.x = value.x();
    out.y = value.y();
    out.z = value.z();
...
hw::Twist FromProto(const cyberbrick_hw_pb::Twist& twist) {
    hw::Twist out;
...
class HardwareService final : public cyberbrick_hw_pb::CyberbrickHardware::Service {
public:
    explicit HardwareService(hw::MotorController controller) : controller_(std::move(controller)) {}

    grpc::Status GetStatus(grpc::ServerContext*, const cyberbrick_hw_pb::GetStatusRequest*, cyberbrick_hw_pb::HardwareStatus* response) override {
    ...
    grpc::Status ApplyVelocity(
        grpc::ServerContext*,
        const cyberbrick_hw_pb::ApplyVelocityRequest* request,
        cyberbrick_hw_pb::ApplyVelocityResponse* response
    ) override {
        std::lock_guard<std::mutex> lock(mutex_);
        try {
    ...
    grpc::Status Stop(grpc::ServerContext*, const cyberbrick_hw_pb::StopRequest*, cyberbrick_hw_pb::StopResponse* response) override {
        std::lock_guard<std::mutex> lock(mutex_);
        try {
...
int main(int argc, char** argv) {
    try {
        ...
        if (parsed.show_version) {
            std::cout << "txing-cyberbrick-hardware-worker "
                      << hw::kTxingCyberbrickHardwareWorkerVersion
                      << "\n";
            return 0;
        }
        ...
        if (!server) {
            throw std::runtime_error("failed to start CyberbrickHardware gRPC server");
        }

        std::cout << "INFO txing-cyberbrick-hardware-worker started version="
                  << hw::kTxingCyberbrickHardwareWorkerVersion
        ...
        std::cout << "INFO txing-cyberbrick-hardware-worker shutdown requested\n";
        service.Shutdown();
        server->Shutdown();
        ...
        std::cerr << "ERROR txing-cyberbrick-hardware-worker: " << err.what() << "\n";
        return 1;
    }
}

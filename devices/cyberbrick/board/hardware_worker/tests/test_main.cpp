...
void TestCliParsesMotorEnvironment() {
    const std::unordered_map<std::string, std::string> values = {
        {"TXING_HARDWARE_WORKER_SOCKET_PATH", "/tmp/cyberbrick-hardware.sock"},
    ...
    auto parsed = hw::ParseCli({"txing-cyberbrick-hardware-worker"}, [&](const std::string& name) -> std::optional<std::string> {
    ...
    Expect(parsed.config.socket_path == "/tmp/cyberbrick-hardware.sock", "CLI should parse socket path from env");
...
int main() {
    try {
        ...
        std::cout << "txing-cyberbrick-hardware-worker tests passed\n";
        return 0;
    } catch (const std::exception& err) {

...
using txing::board::kvs_master::kTxingCyberbrickKvsMasterVersion;
...
void TestCliRequiresStaticWorkerConfigWithoutBridge() {
    bool threw = false;
    try {
        ParseCli({"txing-cyberbrick-kvs-master"}, EnvFrom({}));
...
void TestVersionParsing() {
    const auto parsed = ParseCli({"txing-cyberbrick-kvs-master", "--version"}, EnvFrom({}));
    ...
    Expect(
        std::string(kTxingCyberbrickKvsMasterVersion).find('.') != std::string::npos,
...
void TestCredentialResolution() {
    ...
    const auto temp_dir = std::filesystem::temp_directory_path() / "txing-cyberbrick-kvs-master-tests";
...
void TestRuntimeReadyAndTimestamps() {
    ...
    Expect(
        output.find("version=" + std::string(kTxingCyberbrickKvsMasterVersion)) != std::string::npos,
...
void TestCameraProbeCliDoesNotRequireStaticWorkerConfig() {
    const auto parsed = ParseCli({"txing-cyberbrick-kvs-master", "--camera-probe"}, EnvFrom({}));

...
func TestDaemonEnvTemplateContainsForwardRuntimeDefaults(t *testing.T) {
	...
	// them as literals; the daemon falls back to these defaults instead.
	if DefaultBoardVideoBridgeSocket != "/run/"+DaemonBinaryName+"/board-video-bridge.sock" {
		t.Fatalf("unexpected board video bridge socket default %q", DefaultBoardVideoBridgeSocket)
	}
	if DefaultHardwareSocketPath != "/run/"+HardwareWorkerBinaryName+"/"+DeviceType+"-hardware.sock" {
		t.Fatalf("unexpected hardware worker socket default %q", DefaultHardwareSocketPath)
	}
...
func TestCLIDefaultsToDeclaredBoardCapabilities(t *testing.T) {

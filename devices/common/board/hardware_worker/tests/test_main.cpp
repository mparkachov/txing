...
void TestCliParsesMotorEnvironment() {
    const std::unordered_map<std::string, std::string> values = {
        {"TXING_HARDWARE_WORKER_SOCKET_PATH", "/tmp/board-hardware.sock"},
    ...
    auto parsed = hw::ParseCli({TXING_BOARD_HARDWARE_WORKER_BINARY_NAME}, [&](const std::string& name) -> std::optional<std::string> {
    ...
    Expect(parsed.config.socket_path == "/tmp/board-hardware.sock", "CLI should parse socket path from env");
...
int main() {
    try {
        ...
        std::cout << TXING_BOARD_HARDWARE_WORKER_BINARY_NAME " tests passed\n";
        return 0;
    } catch (const std::exception& err) {

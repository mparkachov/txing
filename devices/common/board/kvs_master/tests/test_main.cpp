...
void TestCliRequiresStaticWorkerConfigWithoutBridge() {
    bool threw = false;
    try {
        ParseCli({TXING_BOARD_KVS_MASTER_BINARY_NAME}, EnvFrom({}));
...
void TestVersionParsing() {
    const auto parsed = ParseCli({TXING_BOARD_KVS_MASTER_BINARY_NAME, "--version"}, EnvFrom({}));
...
void TestCredentialResolution() {
    ...
    const auto temp_dir = std::filesystem::temp_directory_path() / TXING_BOARD_KVS_MASTER_BINARY_NAME "-tests";

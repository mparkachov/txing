...
BUILD_RECIPE_STAMP = BUILD_DIR / ".txing-power-ncs-build-recipe"
...
def west_command() -> list[str]:
    ...
    if not west.exists():
        fail(f"west is missing from {VENV_DIR}; run `just power::mcu::install`")
    return [str(west)]
...
def verify_local_install() -> None:
    ensure_submodule()
    if not (NCS_DIR / ".west").is_dir():
        fail("missing NCS west workspace; run `just power::mcu::install`")
    if not ZEPHYR_BASE.is_dir():
        fail("missing NCS Zephyr checkout; run `just power::mcu::install`")
    if not (SDK_DIR / "arm-zephyr-eabi" / "bin" / "arm-zephyr-eabi-gcc").exists():
        fail("missing local Zephyr SDK arm toolchain; run `just power::mcu::install`")
    if not VENV_PYTHON.exists():
        fail("missing NCS Python environment; run `just power::mcu::install`")
...
def build_power(*, pristine: bool) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build txing power MCU with NCS.")

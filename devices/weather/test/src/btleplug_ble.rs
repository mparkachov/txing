...
use crate::protocol::{
    decode_power_measurement, decode_state, decode_weather_measurement, encode_command,
    power_measurement_uuid, redcon_command_uuid, redcon_service_uuid, redcon_state_uuid,
...
pub struct BtleplugBleCentral {
    ...
    pending_notifications: VecDeque<ValueNotification>,
...
impl BtleplugBleCentral {
            ...
            for peripheral in peripherals {
                ...
                if name_matches {
                        ...
                        .or_else(|| self.last_rssi_by_address.get(&address).copied())
                    {
                        ...
                        None => fields.push(("rssiSource", "unavailable".to_string())),
                    }
                    fields.push((
                        "service",
    ...
    async fn cleanup_after_failed_attempt(&mut self) {
        ...
        self.weather_measurement_char = None;
    ...
    async fn connect_attempt(
        ...
        let power_uuid = power_measurement_uuid();
        let weather_uuid = weather_measurement_uuid();
        ...
        let power_measurement_char = characteristics
            .iter()
            .find(|characteristic| characteristic.uuid == power_uuid)
            .cloned();
        let weather_measurement_char = characteristics
            .iter()
            .find(|characteristic| characteristic.uuid == weather_uuid)
            .cloned();
        events.emit(
            "services",
            &[
                ...
                (
                    "weatherMeasurement",
                    (weather_measurement_char.is_some() as u8).to_string(),
                ),
                (
                    "servicesMs",
        ...
        let weather_measurement_char = weather_measurement_char.ok_or_else(|| {
            RigError::new(
                "services",
                "required weather measurement characteristic is missing",
            )
        })?;
        ...
        peripheral
            .subscribe(&weather_measurement_char)
            .await
            .map_err(|err| {
                RigError::new(
                    "notify",
                    format!("failed to subscribe weather measurement: {err}"),
                )
            })?;
        events.emit(
            "notify",
            &[
                ("characteristic", "weatherMeasurement".to_string()),
                ("enabled", "1".to_string()),
            ],
        );
        ...
        self.weather_measurement_char = Some(weather_measurement_char);
        ...
        let _ = self.read_weather_measurement().await?;
        Ok(())
    }
    ...
    async fn next_matching_notification(
        ...
        loop {
            if let Some(index) = self
                ...
                .iter()
                .position(|notification| notification.uuid == uuid)
            {
                let notification = self
                    .pending_notifications
                    .remove(index)
                    .expect("pending notification index exists");
                return Ok((Instant::now(), notification.value));
            }
            if Instant::now() >= deadline {
                return Err(RigError::new(stage, format!("{stage} deadline expired")));
            }
            ...
            match tokio::time::timeout(wait, next)
                .await
                .map_err(|_| RigError::new(stage, format!("{stage} deadline expired")))?
            {
                Some(notification) if notification.uuid == uuid => {
                    return Ok((Instant::now(), notification.value));
                }
...
impl BleCentral for BtleplugBleCentral {
    ...
    async fn read_weather_measurement(&mut self) -> Result<TimedWeatherMeasurement> {
        let peripheral = self
            ...
            .as_ref()
            .ok_or_else(|| RigError::new("connect", "not connected"))?;
        let characteristic = self.weather_measurement_char.as_ref().ok_or_else(|| {
            RigError::new("services", "weather measurement characteristic missing")
        })?;
        let payload = peripheral.read(characteristic).await.map_err(|err| {
            RigError::new(
                "weather-measurement",
                format!("failed to read weather measurement: {err}"),
            )
        })?;
        Ok(TimedWeatherMeasurement {
            received_at: Instant::now(),
            measurement: decode_weather_measurement(&payload)?,
        })
    }
    ...
    async fn next_state(&mut self, timeout: Duration) -> Result<TimedState> {
        let (received_at, payload) = self
            .next_matching_notification(redcon_state_uuid(), timeout, "state")
            .await?;
        Ok(TimedState {
            received_at,
            state: decode_state(&payload)?,
    ...
    async fn next_power_measurement(&mut self, timeout: Duration) -> Result<TimedPowerMeasurement> {
        let (received_at, payload) = self
            .next_matching_notification(power_measurement_uuid(), timeout, "power measurement")
            .await?;
        Ok(TimedPowerMeasurement {
            received_at,
            measurement: decode_power_measurement(&payload)?,
    ...
    async fn next_weather_measurement(
        ...
        let (received_at, payload) = self
            .next_matching_notification(weather_measurement_uuid(), timeout, "weather measurement")
            .await?;
        Ok(TimedWeatherMeasurement {
            received_at,
            measurement: decode_weather_measurement(&payload)?,
    ...
    async fn close(&mut self) -> Result<()> {
        ...
        if let (Some(peripheral), Some(characteristic)) =
            (&self.peripheral, &self.weather_measurement_char)
        {
            let _ = peripheral.unsubscribe(characteristic).await;
        }
        if let Some(peripheral) = self.peripheral.take()

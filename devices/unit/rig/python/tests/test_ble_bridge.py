...
class AwsShadowClientTests(unittest.TestCase):
    ...
    def test_justfile_install_service_uses_greengrass_supervision(self) -> None:
        ...
        self.assertIn('core_runtime_units=("${core_sockets[@]}" "${core_units[@]}")', justfile)
        self.assertIn("Greengrass Lite core runtime $core_runtime_unit is not installed", justfile)
        self.assertIn("sudo systemctl start \"$core_runtime_unit\"", justfile)
        self.assertIn("sudo journalctl -a --no-pager -u $core_runtime_unit -u ggl.core.ggdeploymentd.service -u ggl.gg_deployment.socket -n 200", justfile)

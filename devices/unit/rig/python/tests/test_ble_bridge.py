...
class AwsShadowClientTests(unittest.TestCase):
    ...
    def test_justfile_install_service_uses_greengrass_supervision(self) -> None:
        ...
        self.assertNotIn("--refresh-package aws --reinstall-package aws", justfile)
        ...
        self.assertNotIn("Missing built rig entrypoint", justfile)
        ...
        self.assertIn("start rig_id='' components='auto':", justfile)
        self.assertIn("stop rig_id='':", justfile)
        self.assertIn("restart rig_id='' components='auto':", justfile)
        self.assertIn('_local-start rig_id=\'\' components=\'auto\':', justfile)
        ...
        self.assertIn('greengrass_warning_log_units := "ggl.core.ggipcd.service ggl.core.iotcored.service"', justfile)
        self.assertIn("install_core_log_overrides() {", justfile)
        self.assertIn("LogLevelMax=warning", justfile)
        self.assertIn('sudo systemctl try-restart "$log_unit"', justfile)
        self.assertIn("ggl.core.tesd.service", justfile)
        self.assertIn("ggl.core.ggdeploymentd.service", justfile)
        self.assertIn("greengrass_core_sockets :=", justfile)
        self.assertIn("ggl.gg_deployment.socket", justfile)
        self.assertNotIn("'ggl.*.service'", justfile)
        self.assertNotIn("'ggl.*.socket'", justfile)
        ...
        self.assertIn("verify_local_deployment_if_targeted() {", justfile)
        self.assertIn("unit_loaded_version_matches() {", justfile)
        self.assertIn("TXING_RIG_DEPLOY_WAIT_SECONDS", justfile)
        self.assertIn("No local Greengrass Lite target is installed; skipping local deployment verification.", justfile)
        self.assertIn("Local Greengrass Lite is installed, but local rig type could not be resolved", justfile)
        self.assertIn("waiting for local Greengrass deployment target=%s version=%s units:", justfile)
        self.assertIn("Greengrass deployment did not update local systemd units to version", justfile)
        self.assertIn("---- Greengrass deployment logs ----", justfile)
        self.assertIn("deploy-local rig_id=''", justfile)
        self.assertNotIn("aws_shared_credentials_file=aws_shared_credentials_file:", justfile)
        self.assertNotIn("Run 'just rig::build' before 'just rig::" "deploy'", justfile)
        self.assertIn("Greengrass Lite target {{greengrass_lite_target}} is not installed", justfile)
        self.assertIn(
            "Starting Greengrass deployment sockets before deploy; component target {{greengrass_lite_target}} will be activated after deployment.",
            justfile,
        )
        self.assertIn("sudo install -d -o ggcore -g ggcore -m 775 /run/greengrass", justfile)
        ...
        self.assertIn("wait_unit_version() {", justfile)
        self.assertIn("reset_failed_if_present() {", justfile)
        self.assertIn("prune_component_config_arn() {", justfile)
        self.assertIn("/var/lib/greengrass/config.db", justfile)
        self.assertIn("Json encode failed for key services/<component>/configArn", justfile)
        self.assertIn("txing-configArn-backup", justfile)
        self.assertIn('prune_local_deployment_config_arns "$sparkplug_component" "$connectivity_component"', justfile)
        self.assertIn("core_units=({{greengrass_core_units}})", justfile)
        self.assertIn("core_sockets=({{greengrass_core_sockets}})", justfile)
        self.assertIn("ggl.gg_config.socket", justfile)
        self.assertIn('core_runtime_units=("${core_sockets[@]}")', justfile)
        self.assertIn("is_deployment_required_core_unit() {", justfile)
        self.assertIn("ggl.gg_config.socket|ggl.gg_deployment.socket", justfile)
        self.assertIn("Greengrass Lite deployment runtime $core_runtime_unit is not installed", justfile)
        self.assertIn('reset_failed_if_present "$core_runtime_unit"', justfile)
        self.assertIn("sudo systemctl start \"$core_runtime_unit\"", justfile)
        self.assertIn("Greengrass Lite deployment runtime $core_runtime_unit did not become active", justfile)
        self.assertIn("warning: Greengrass Lite socket $core_runtime_unit did not report active before deploy", justfile)
        self.assertIn("sudo journalctl -a --no-pager -u $core_runtime_unit -u ggl.gg_config.socket -u ggl.gg_deployment.socket -u ggl.core.ggdeploymentd.service -n 200", justfile)
        self.assertIn("Stopping running Greengrass component units before deploy", justfile)
        self.assertIn('sudo systemctl stop "${quiesce_units[@]}" || true', justfile)
        self.assertIn('if ! wait_inactive_unit "$unit"; then', justfile)
        ...
        self.assertNotIn('unset AWS_PROFILE AWS_DEFAULT_PROFILE AWS_SHARED_CREDENTIALS_FILE', justfile)
        ...
        self.assertIn("systemd still points at a different component version after waiting", justfile)
    ...
    def test_greengrass_templates_are_rig_local(self) -> None:
        ...
        self.assertIn("aws.greengrass.ipc.mqttproxy", ble_connectivity_recipe)
        self.assertIn("aws.greengrass#PublishToIoTCore", ble_connectivity_recipe)
        self.assertIn("$aws/things/+/shadow/name/ble/update", ble_connectivity_recipe)
        self.assertIn("$aws/things/+/shadow/name/power/update", ble_connectivity_recipe)
        self.assertIn("$aws/things/+/shadow/name/weather/update", ble_connectivity_recipe)

...
class AwsShadowClientTests(unittest.TestCase):
    ...
    def test_justfile_install_service_uses_greengrass_supervision(self) -> None:
        ...
        self.assertIn("deployment_runtime_units=(ggl.gg_deployment.socket ggl.core.ggdeploymentd.service)", justfile)
        self.assertIn("Greengrass Lite deployment runtime $deployment_runtime_unit is not installed", justfile)
        self.assertIn("sudo systemctl start \"$deployment_runtime_unit\"", justfile)
        self.assertIn("sudo journalctl -a --no-pager -u ggl.core.ggdeploymentd.service -u ggl.gg_deployment.socket -n 200", justfile)

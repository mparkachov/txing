...
def cross_compile_prefix() -> str:
	configured = os.environ.get("POWER_DEBUG_CROSS_COMPILE") or os.environ.get("CROSS_COMPILE")
	if configured:
		return configured
...
def openocd_executable() -> str:
	...
	if shutil.which(openocd) is None:
		raise SystemExit(
			"missing OpenOCD for power-debug flash.\n"
...
def command_env() -> dict[str, str]:
	...
	local["POWER_DEBUG_BOARD_ROOT"] = str(BOARD_ROOT)
...
def ensure_toolchain() -> None:
	...
	if missing:
		...
		raise SystemExit(
			...
			"Or set POWER_DEBUG_CROSS_COMPILE to a full prefix such as "
			"/opt/homebrew/bin/arm-none-eabi-\n"
			"Missing:\n"
			f"{paths}"
		)
...
def install() -> None:
	...
	if not python_executable().exists():
		print(f"creating power-debug Zephyr venv: {VENV_DIR}", flush=True)
...
def main() -> None:
	...
	if args.command == "submodules":
		...
		flash()
	elif args.command == "flash-command":
		flash_command()

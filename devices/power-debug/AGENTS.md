...
## Hardware Rules
- Agents may run build/setup commands such as `just power-debug::mcu::submodules`, `just power-debug::mcu::install`, `just power-debug::mcu::check`, `just power-debug::mcu::build`, `just power-debug::mcu::check-flash`, `just power-debug::mcu::paths`, and `just power-debug::mcu::clean`.

...
def native_cmake_configure() -> None:
	...
	run(
		[
			...
			f"-DBUILD_VERSION={NATIVE_BUILD_VERSION}",
...
def native_flash_openocd_command() -> list[Path | str]:
	if not NATIVE_FIRMWARE_HEX.exists():
		raise SystemExit(
			"missing native firmware hex. Run: just power-debug::firmware-native-check"
		)
	...
	return [
		...
		"reset init",
		"-c",
		f"nrf54l-load {NATIVE_FIRMWARE_HEX}",
		"-c",
		f"verify_image {NATIVE_FIRMWARE_HEX}",
		"-c",
		"reset run",
		"-c",
		"shutdown",

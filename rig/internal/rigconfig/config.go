...
func Load(configDirOverride string) (Config, error) {
	...
	cfg := Config{
		...
		CertificateFile:         firstNonEmpty(lookup("TXING_IOT_CERT_FILE"), filepath.Join(configDir, "certificate.pem.crt"), filepath.Join(configDir, "rig.cert.pem")),
		PrivateKeyFile:          firstNonEmpty(lookup("TXING_IOT_PRIVATE_KEY_FILE"), filepath.Join(configDir, "private.pem.key"), filepath.Join(configDir, "rig.private.key")),
